...
class Tweet(val user: String, val text: String, val retweets: Int) {
  def > (max: Tweet):Boolean = this.retweets>max.retweets
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    def descendingByRetweet(tweetSet: TweetSet, tweetList: TweetList): TweetList = {
      if(tweetSet.isEmpty) tweetList
        ...
        descendingByRetweet(tweetSet.remove(most), new Cons(most, tweetList))
      }
    }
  ...
  override def mostRetweeted: Tweet = {
    def max(tweetA: Tweet, tweetB:  Tweet) = if(tweetA.retweets<tweetB.retweets) tweetA else tweetB
...
object GoogleVsApple {
  ...
  def mentionedTweets(tweetSet: TweetSet, keywords: List[String]) =
    tweetSet.filter(t => keywords.exists(s => t.text.contains(s)))


    lazy val googleTweets: TweetSet = mentionedTweets(TweetReader.allTweets,google)
    lazy val appleTweets:  TweetSet = mentionedTweets(TweetReader.allTweets,apple)
