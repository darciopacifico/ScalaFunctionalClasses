...
object LineOfSight {
   ...
   * of the angle. Indeed, xs(i) is the opposing side of the angle and i the
   * adjacent side. The ratio xs(i) / i that you compute is in fact the tangent
   * of the angle! Since the tangent of an angle is strictly increasing
   * between 0° and 90°, it is a perfectly good replacement for the actual angle in
   * our use case. Keep this in mind and make sure that you do not apply any
   * trigonometic functions on the tangent!
   *
   * @param input
   * @param output
   */
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      ...
      case Node(l, r) =>
        val m = max(l.maxPrevious, startingAngle)
        parallel(downsweep(input, output, startingAngle, l), downsweep(input, output, m, r))
...
*
      ...
      case Node(null, r) =>
        val m = max(tree.maxPrevious,startingAngle)
        downsweep(input, output, m, r)

      case Node(l, null) =>
        downsweep(input, output, startingAngle, l)
      case _ =>
        println("bizarre non expected case")
